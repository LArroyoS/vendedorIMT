...
/*================================================*/
    ...
    public function header(){
        ...
        $this->Cell(0,10,utf8_decode("REFACCIONARIA"),0,0,'C');
        $this->Ln();
        $this->SetTextColor(255,255, 255);
        $this->SetFont('rockwell','B',26);
        $this->Cell(0,10,utf8_decode("COTIZACION"),0,0,'C');
        $this->SetFont('junegull','B',45);
        $this->Ln(22);
    ...
    public function footer(){
        ...
        $this->SetFillColor(12,52,67);
        $this->Rect(0,275,216,22, 'F');

        $this->SetY(-17);
        $this->SetTextColor(255, 255, 255);
        $this->SetFont('calibri','',12);
        $this->Cell(0,6,utf8_decode("AV. ANACAHUITA NO.266. COL. SANTO DOMINGO, COYOACÁN."));
        $this->Ln();
        $this->Cell(0,6,utf8_decode("TELEFONO: 5517357208"));
    ...
    function SetFontTable($f){

        $this->font=$f;

    }

    function SetStyleFontTable($sf){

        $this->fontStyle=$sf;

    }

    function SetTamFontTable($t){
    ...
    function SetbordeYrelleno($br)
    {
        //Set the array of column alignments
        $this->bordeYrelleno=$br;
    }
    ...
    function Row($data)
    {
        ...
        for($i=0;$i<count($data);$i++)
        {
            ...
            $f=isset($this->font[$i]) ? $this->font[$i] : 'Arial';
            $sf=isset($this->fontStyle[$i]) ? $this->fontStyle[$i] : '';
            $t=isset($this->fontTam[$i]) ? $this->fontTam[$i] : '10';
            $br=isset($this->bordeYrelleno[$i]) ? $this->bordeYrelleno[$i] : '';
            //Save the current position
            $x=$this->GetX();
            $y=$this->GetY();
            //Draw the border

            $this->Rect($x,$y,$w,$h,$br);
            //Print the text
            $this->SetFont($f,$sf,$t);
